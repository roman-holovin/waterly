...
const withViewportHandlers = withHandlers({
  selectNext: ({ selectedIndex, days, onDaySelect }) => (e) => {
    e.preventDefault();
    if (selectedIndex < days.length - 1) {
      onDaySelect(days[selectedIndex + 1]);
    }
  },
  selectPrev: ({ selectedIndex, days, onDaySelect }) => (e) => {
    e.preventDefault();
    if (selectedIndex > 0) {
      onDaySelect(days[selectedIndex - 1]);
    }
...
const dummy = key => <li className="timeline__day timeline__day--dummy" key={key} />;
...
function Timeline({ days, selectedIndex, selectNext, selectPrev }) {
  return (
    <nav>
      <Swipeable
        nodeName="ol"
